<script>
	...
	let generalItems = [
		{ name: 'Support', icon: '../../navbar/support.svg' },
		{ name: 'Settings', icon: '../../navbar/settings.svg' },
		{ name: 'Tutorials', icon: '../../navbar/tutorial.svg' }
	];
</script>
...
<div class="p-4 mb-12">
	<div class="mb-4">
		...
		<ul>
			...
			{/each}
			<li
				class="flex items-center text-red-500 cursor-pointer ${isCollapsed ? 'justify-center' : ''}"
			>
	...
	<button
		class={`w-full bg-primary-500 text-white py-2 rounded-lg flex items-center justify-evenly mt-4 ${isCollapsed ? 'hidden' : 'block'}`}
	>
