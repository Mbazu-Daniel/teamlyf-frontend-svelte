...
<table class="min-w-full bg-white">
	...
	<tbody>
		{#each tasks as task}
			<tr class="border-b border-gray-200 cursor-pointer" on:click={() => openTaskModal(task)}>
				...
				<td class="py-4 px-4 text-sm text-gray-800">
					<div class="flex items-center">
						...
						{#if task.assignees.length > 3}
							<span class="ml-2 text-gray-500">+{task.assignees.length - 3}</span>
						{/if}
					</div>
				</td>
				...
				<td class="py-4 px-4 text-sm text-gray-800">
					<div class="flex space-x-2">
						<span class="flex items-center bg-gray-300 rounded-3xl p-1">
							<img src="../../images/subtasks.svg" alt="Subtasks" class="w-6 h-6" />
						</span>
						<span class="flex items-center">
							<img src="../../images/comments.svg" alt="Comments" class="w-7 h-7" />
							{task.comments}
						</span>
						<span class="flex items-center">
							<img src="../../images/attachments.svg" alt="Attachments" class="w-6 h-6" />
							{task.attachments}
						</span>
						<button class="px-2 py-1 bg-primary-500 text-white rounded-md">✓</button>
